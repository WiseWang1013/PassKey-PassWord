...
# ===================== 格式映射：Mac + Linux，移除Windows bat/cmd =====================
TEXT_FORMATS = {
    ...
    # Linux专属
    "sh": ["reverse", "caesar"],
    "conf": ["caesar", "urlencode"],
    "service": ["base64", "reverse"],
    # macOS专属格式
    "plist": ["base64", "reverse"],
    "mobileconfig": ["caesar", "urlencode"]
}
...
class MultiFileCryptoGUI:
    def __init__(self,root):
        self.root = root
        self.root.title("PassKey v1.1.1-beta-for-macOS 多格式文件加密工具")
        ...
        style.configure("TLabel", background=bg_color, foreground="#adbac7", font=font_small)
        style.configure("Status.TLabel", background=bg_color, font=font_bold)
        ...
        tk.Label(root, text=tip_text, fg="#58a6ff", bg=bg_color, wraplength=990, font=font_small).pack(pady=4)
        ...
        tk.Label(progress_frame,text="进度：",bg=bg_color,fg="#adbac7",font=font_main).pack(side="left")
        ...
        self.status = tk.Label(root,text="就绪，可以直接在文本框打字，或拖拽文件",fg="#3fb950",bg=bg_color,font=font_bold)
